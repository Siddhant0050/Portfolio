...
const Navbar = ({ isDark, setIsDark }) => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-[100] transition-colors duration-500 border-b pb-safe ${
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8 h-20 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4 md:hidden">
           <button onClick={() => setIsDark(!isDark)} className={isDark ? 'text-white' : 'text-black'}>
        ...
        <div className="hidden md:flex items-center gap-10">
          ...
          <button onClick={() => setIsDark(!isDark)} className={`p-2 rounded-full ${isDark ? 'text-white' : 'text-black'}`}>
      ...
      {/* Mobile Menu Overlay */}
      {isOpen && (
        <div className={`md:hidden absolute top-20 left-0 w-full p-8 space-y-6 border-b animate-in slide-in-from-top ${
          isDark ? "bg-[#050505] border-white/5" : "bg-white border-black/5"
        }`}>
          {['Home', 'Experience', 'Education', 'Skills', 'Projects', 'Contact'].map((item) => (
            <a key={item} href={`#${item.toLowerCase()}`} onClick={() => setIsOpen(false)} className="block text-[12px] font-bold uppercase tracking-widest">
              {item}
            </a>
          ))}
        </div>
